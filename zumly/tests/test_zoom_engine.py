...
from app.zoom_engine import ease_in_out, ease_out, smooth_step, speed_at_time, ZoomEngine, MAX_UNDO
from app.models import ClickEvent, TextAnnotation, ZoomKeyframe
...
class TestEaseOut:
    ...
    def test_smooth_step_alias(self) -> None:
        assert smooth_step is ease_in_out


class TestEaseInOut:
    def test_quintic_smoothstep_midpoint(self) -> None:
...
class TestCanvasTextAndCursorUndoRedo:
    def test_deleted_text_restores_exact_state(self) -> None:
        engine = ZoomEngine()
        ...
        engine.text_annotations = [annotation]
        engine.push_undo("Delete canvas text")
        engine.text_annotations = []

        assert engine.undo()
        assert engine.text_annotations == [annotation]
        assert engine.redo()
        assert engine.text_annotations == []

    def test_cursor_assignment_is_reversible(self) -> None:
        engine = ZoomEngine()
        engine.cursor_asset_path = "old.png"
        engine.push_undo("Change cursor")
        engine.cursor_asset_path = "new.png"

        assert engine.undo()
        assert engine.cursor_asset_path == "old.png"
        assert engine.redo()
        assert engine.cursor_asset_path == "new.png"

    def test_dismissed_pause_suggestions_are_reversible(self) -> None:
        engine = ZoomEngine()
        engine.dismissed_screen_transition_ids = ["pause-old"]
        engine.push_undo("Dismiss screen transition")
        engine.dismissed_screen_transition_ids = ["pause-old", "pause-new"]

        assert engine.undo()
        assert engine.dismissed_screen_transition_ids == ["pause-old"]
        assert engine.redo()
        assert engine.dismissed_screen_transition_ids == ["pause-old", "pause-new"]
